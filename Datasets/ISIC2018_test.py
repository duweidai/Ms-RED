...
class ISIC2018_dataset(Dataset):
    def __init__(self, dataset_folder='/ISIC2018_Task1_npy_all',
                 test_list='ISIC2017_Test_v2.list', train_type='train', suffix = "_segmentation.npy", transform=None):
        self.transform = transform
        self.train_type = train_type
        self.folder_file = './Datasets/folder0/' + test_list
        ...
        if self.train_type in ['train', 'validation', 'test']:
            ...
            self.mask = [join(dataset_folder, 'label', x.split('.')[0] + suffix) for x in self.image_list]
