...
class SoftDiceLoss(_Loss):
    ...
    def forward(self, prediction, soft_ground_truth, num_class=3, weight_map=None, eps=1e-8):
        dice_loss_ave, dice_loss_back, dice_loss_lesion = soft_dice_loss(prediction, soft_ground_truth, num_class, weight_map)
        return dice_loss_ave, dice_loss_back, dice_loss_lesion
...
def soft_dice_loss(prediction, soft_ground_truth, num_class, weight_map=None):
    ...
    dice_loss_back = dice_loss[0]
    dice_loss_lesion = dice_loss[1]
    return dice_loss_ave, dice_loss_back, dice_loss_lesion
...
def IOU_hard_loss(prediction, soft_ground_truth, num_class):
    predict = prediction.permute(0, 2, 3, 1)
