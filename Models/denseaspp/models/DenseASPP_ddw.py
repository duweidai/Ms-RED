...
class DenseASPP_121(nn.Module):
    """
    * output_scale can only set as 8 or 16
    """
    def __init__(self, n_class=2, output_stride=8):
        super(DenseASPP_121, self).__init__()
        bn_size = 4
        drop_rate = 0
        growth_rate = 32
    ...
    def forward(self, _input):
        feature = self.features(_input)                   # [12,3, 224, 320]-->[12, 512, 28, 40]

        aspp3 = self.ASPP_3(feature)                      # [12, 64, 28, 40]
        feature = torch.cat((aspp3, feature), dim=1)      # [12, 576, 28, 40]

        aspp6 = self.ASPP_6(feature)                      # [12, 64, 28, 40]
        feature = torch.cat((aspp6, feature), dim=1)      # [12, 640, 28, 40]

        aspp12 = self.ASPP_12(feature)                    # [12, 64, 28, 40]
        feature = torch.cat((aspp12, feature), dim=1)     # [12, 704, 28, 40]

        aspp18 = self.ASPP_18(feature)                    # [12, 64, 28, 40]
        feature = torch.cat((aspp18, feature), dim=1)     # [12, 768, 28, 40]

        aspp24 = self.ASPP_24(feature)
        feature = torch.cat((aspp24, feature), dim=1)     # [12, 832, 28, 40]
...
class DenseASPP_161(nn.Module):
    """
    * output_scale can only set as 8 or 16
    """
    def __init__(self, n_class=2, output_stride=8):
        super(DenseASPP_161, self).__init__()
        bn_size = 4
        drop_rate = 0
        growth_rate = 48
